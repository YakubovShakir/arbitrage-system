...
use crate::{
    ...
    utils::{fetch_data, parse_json_glass_as_number},
};
...
impl ExchangeAPI for Huobi {
    ...
    async fn fetch_order_book(&self, base: &str, quote: &str) -> Result<OrderBook, Box<dyn Error>> {
        let result = fetch_data(
            ...
            self.base_url().to_string() + "/market/depth",
            &[
                (
                    "symbol".to_string(),
                    (base.to_string() + quote).to_lowercase(),
                ),
                ("depth".to_string(), "20".to_string()),
                ("type".to_string(), "step0".to_string()),
            ],
        )
        .await?;
        if result["status"] != "ok" || !result.has_key("tick") {
            return Err(format!("Fail fetch {} order book", self.name()).into());
        }

        let raw_asks = &result["tick"]["asks"];
        let raw_bids = &result["tick"]["bids"];

        if !raw_asks.is_array() || !raw_bids.is_array() {
            return Err("Invalid response: 'asks' or 'bids' is not an array".into());
        }
        let formated_asks: Asks = parse_json_glass_as_number(&raw_asks)?;
        let formated_bids: Bids = parse_json_glass_as_number(&raw_bids)?;
